...
def run(redshift, pkinit, f=0.0, boxsize=512.0, ngrid=512, exactpk=True, smw=2.0, seed=314159):
    ...
    dens0 = make_gauss_init(pkinit, boxsize=boxsize,
                            ngrid=ngrid, seed=seed, exactpk=exactpk, smw=smw)
    ...
    x, y, z = get_pos(fx, fy, fz * (1 + f), redshift,
...
def make_gauss_init(pkinit, boxsize=512.0, ngrid=512, seed=314159, exactpk=True, smw=2.0):
    ...
    if (exactpk):
        dk = N.exp(2j * N.pi * rs.rand(ngrid * ngrid * (thirdim))
                   ).reshape((ngrid, ngrid, thirdim)).astype(N.complex64)
    else:
        dk = N.empty((ngrid, ngrid, thirdim), dtype=N.complex64)
        dk.real = rs.normal(size=ngrid * ngrid * (thirdim)
                            ).reshape((ngrid, ngrid, thirdim)).astype(N.float32)
        dk.imag = rs.normal(size=ngrid * ngrid * (thirdim)
                            ).reshape((ngrid, ngrid, thirdim)).astype(N.float32)
        dk /= N.sqrt(2.0)
    ...
    if (kgrid[0, 0, 0] == 0):
        ...
        dk[wn0] *= N.sqrt(filt[wn0] * pkinterp(kgrid[wn0])) * \
            ngrid**3 / boxsize**1.5
    else:
        dk *= N.sqrt(filt * pkinterp(kgrid.flatten())
                     ).reshape(sk) * ngrid**3 / boxsize**1.5
    dk = nyquist(dk)
...
def nyquist(xp):
    ngrid = xp.shape[0]
    xp[ngrid / 2 + 1:, 1:,
        0] = N.conj(N.fliplr(N.flipud(xp[1:ngrid / 2, 1:, 0])))
    xp[ngrid / 2 + 1:, 0, 0] = N.conj(xp[ngrid / 2 - 1:0:-1, 0, 0])
    xp[0, ngrid / 2 + 1:, 0] = N.conj(xp[0, ngrid / 2 - 1:0:-1, 0])
    xp[ngrid / 2, ngrid / 2 + 1:,
        0] = N.conj(xp[ngrid / 2, ngrid / 2 - 1:0:-1, 0])

    xp[ngrid / 2 + 1:, 1:, ngrid /
        2] = N.conj(N.fliplr(N.flipud(xp[1:ngrid / 2, 1:, ngrid / 2])))
    xp[ngrid / 2 + 1:, 0, ngrid /
        2] = N.conj(xp[ngrid / 2 - 1:0:-1, 0, ngrid / 2])
    xp[0, ngrid / 2 + 1:, ngrid /
        2] = N.conj(xp[0, ngrid / 2 - 1:0:-1, ngrid / 2])
    xp[ngrid / 2, ngrid / 2 + 1:, ngrid /
        2] = N.conj(xp[ngrid / 2, ngrid / 2 - 1:0:-1, ngrid / 2])
    return xp
