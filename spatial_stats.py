...
# This module computes the power spectrum and correlation function of a
# given 3d density grid
...
# exp_smooth is the size of an exponential smoothing window to deconvolve
# (if = 0.0, then it does nothing)
...
def getPk(density, nkbins=40, boxsize=512., deconvolve_cic=False, exp_smooth=0.0):
    ...
    # Square the density in Fourier space to get the 3D power, make sure k=0
    # mode is 0
    ...
    if (deconvolve_cic):
        wx = N.sin(kmin * a * N.pi / (2 * kny))**4 / \
            (kmin * a * N.pi / (2 * kny))**4
        wx[N.where(a == 0)] = 1.0
        wy = N.sin(kmin * b * N.pi / (2 * kny))**4 / \
            (kmin * b * N.pi / (2 * kny))**4
        wy[N.where(b == 0)] = 1.0
        wz = N.sin(kmin * c * N.pi / (2 * kny))**4 / \
            (kmin * c * N.pi / (2 * kny))**4
        wz[N.where(c == 0)] = 1.0
        ww = wx * wy * wz
        dk2 = dk2 / ww
    ...
    # Now we want to compute the 1-D power spectrum which involves averaging
    # over shells in k-space
    ...
    for i in N.arange(0, nkbins - 1):
        if (cuts[i + 1] > cuts[i]):
            ...
            kmean[i] = N.sum(c0[cuts[i]:cuts[i + 1]] *
                             kgrid[cuts[i]:cuts[i + 1]])
...
def getXi(density, nrbins=40, boxsize=512., get2d=False, deconvolve_cic=False, exp_smooth=0.0):
    ...
    if (get2d):
        ...
        for i in N.arange(ngrid / 2):
            rzmean[i] = N.sum(c0_ind[cutsz[i]:cutsz[
                              i + 1]] * rz[cutsz[i]:cutsz[i + 1]]) / N.sum(c0_ind[cutsz[i]:cutsz[i + 1]])
            ...
            for ic in N.arange(0, ngrid / 2):
                if (cuts[ic + 1] > cuts[ic]):
                    xips[i, ic] = N.sum(c0_indexed[cuts[ic]:cuts[
                                        ic + 1]] * xi3d_sort[cuts[ic]:cuts[ic + 1]]) / N.sum(c0_indexed[cuts[ic]:cuts[ic + 1]])
        ...
        for i in N.arange(0, ngrid / 2):
            rmean[i] = N.sum(c0_indexed[cuts[i]:cuts[
                             i + 1]] * rp_sort[cuts[i]:cuts[i + 1]]) / N.sum(c0_indexed[cuts[i]:cuts[i + 1]])
        ...
        for i in N.arange(0, nrbins - 1):
            if (cuts[i + 1] > cuts[i]):
                numinbin[i] = N.sum(c0[cuts[i]:cuts[i + 1]])
                xir[i] = N.sum(c0[cuts[i]:cuts[i + 1]] *
                               xi3d[cuts[i]:cuts[i + 1]])
                rmean[i] = N.sum(c0[cuts[i]:cuts[i + 1]] *
                                 rgrid[cuts[i]:cuts[i + 1]])
        wn0 = N.where(numinbin > 0.)
        xir = xir[wn0]
        rmean = rmean[wn0]
        numinbin = numinbin[wn0]
